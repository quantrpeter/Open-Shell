...
def pipeline_index() -> int:
    """1-based stage index of the command currently running. First command is 1."""
    return _pipeline_index


def _stage_records(records: Records, index: int) -> Records:
    """Yield from a generator while pinning pipeline_index() to this stage.

    Command functions are generators: `fn()` only constructs them. The body
    runs on first `next()`, often after later stages have already been wired.
    """
...
def iter_pipeline(
	...
	for position, (cmd, args) in enumerate(resolved, start=1):
		...
		if not cmd.source and position == 1 and not has_input:
			raise ShellError("pipe.no_input", f"`{cmd.name}` needs input records",
							 f"e.g. ls | {cmd.name} ...")
		_pipeline_index = position
		result = cmd.fn(stream, args)
		stream = _stage_records(result, position) if inspect.isgenerator(result) else result
	return stream
